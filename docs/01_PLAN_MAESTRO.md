...
**🔨 EN CURSO (2026-07-07) — items 1 y 2 entregados, gate visual PENDIENTE (no se cierra F4):**
- **F4.1 (backend, commit `c7652e3`):** `services/isosurface_service.py` + `GET /v2/isosurface`. Marching cubes (skimage) sobre el campo de CONTRASTE robusto idéntico al del visor (fondo=mediana, escala=max(P95-bg, bg-P5, 0.05)); niveles = fracciones del pico (0.5/0.7/0.9); suavizado Taubin (no encoge volumen); transform al espacio visual Three.js (centrado + flip-Y) **idéntico al transporte Arrow** → superposición pixel-a-pixel con los vóxeles (verificado E2E: iso center == Arrow center); normales orientadas por **voto contra −∇|contraste|** (robusto en mallas ABIERTAS, cuerpo pegado al borde tipo Raglan, donde el volumen firmado no sirve); color por vértice = contraste con signo → mismo Viridis divergente; salida JSON base64 Float32/Uint32; density y susceptibility (log); nunca-crashea. Tests: gota gaussiana con verdad conocida (volumen monótono anidado, centroide, normales hacia afuera), malla abierta, campo uniforme, magnético, bordes (10 tests). `scikit-image` declarado en requirements.
- **F4.2 (frontend, commit `94d4a16`):** `lib/render/IsosurfaceMeshLayer.tsx` (capa r3f auto-contenida que decodifica y dibuja las mallas, transparencia por nivel, planos de corte compartidos, sin calcular física — color = Viridis del contraste del backend) + proxy `/api/isosurface` + store (`isosurfaceData`/`showIsosurfaces`, default **OFF** hasta QA visual) + fetch aislado en Exploration3DView (campo sigue a viewMode) + montaje en Scene3D bajo el mismo grupo centrado + toggle en `VolumeRenderControls`. Verificado: tsc limpio, eslint 0 errores, frontera física respetada.
- **FALTA para cerrar F4:** (gate) QA visual de Martín en los 4 datasets canónicos + medición 30 fps + reviewer `frontend-physics-boundary-reviewer` formal → recién ahí se pone `showIsosurfaces` default ON. Items restantes: **F4.3** cortes arbitrarios con cara pintada (las isosuperficies YA respetan clippingPlanes; falta el raster de la cara + endpoint de sección), **F4.4** sondajes/terreno como contexto (terreno y MVI ya existen), **F4.5** overlay de incertidumbre DOI, **F4.6** WebGPU progresivo (building blocks: `VolumeRaymarchLayer`), **F4.7** LOD octree.
