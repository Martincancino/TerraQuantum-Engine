...
- **NO existe (F2B lo construye)**: corrección de deriva instrumental y marea terrestre desde lecturas crudas (`drift_correction`/`tide_correction` solo se reconocen como columnas que el usuario TRAE, csv_analysis_service.py:73 — no se calculan); separación regional-residual como producto de usuario; corrección diurna magnética; reducción al polo (RTP); derivadas (1VD, THD, tilt, señal analítica); continuación ascendente; deconvolución de Euler; espectro de potencia radial; desurvey de sondajes inclinados (azimut/dip); QA/QC formal de intervalos.
