...
- **Updater firmado:** `tauri-plugin-updater` registrado; `createUpdaterArtifacts:true`; pubkey minisign en `tauri.conf.json`; **clave privada FUERA del repo** (`~/.tauri/terraquantum_updater.key`) firma el instalador → `.sig` generado. ~~Endpoint `latest.json` placeholder (GitHub Releases) a reemplazar por el repo real.~~ **Corregido por la Fase 28 (09-06):** el placeholder no era un hueco inocuo — apuntaba a la cuenta de un **tercero real** — y el aviso presentaba el 404 como falta de internet. Endpoint al remoto real, cinco desenlaces distinguibles y `.github/workflows/release.yml`. **Sigue sin funcionar para un usuario final mientras el repositorio sea privado** (NUEVO-13): ver `docs/04` §6.1.
...
**Pendiente MENOR de F7 (no bloquea; el motor y el empaque están probados):** cronómetro formal del "tercero instala en <15 min" en una VM Windows LIMPIA (sin Python/Node) corriendo el `.exe`; publicar `latest.json` firmado en un repo de releases **público** (el workflow existe desde la Fase 28; lo que falta es la decisión de visibilidad, NUEVO-13); reemplazar iconos placeholder por la marca; opcional `webviewInstallMode: offlineInstaller` si se quiere instalación 100% sin internet. Todo F7 (backend núcleo + Plan B + Tauri) **SIN commitear** (patrón F5/F6, decisión del usuario).
