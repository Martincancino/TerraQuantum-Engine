...
| **3** | **El operador de suavidad es de cuarto orden, no de segundo.** Se penaliza $\|Lm\|^2=m^\top L^2m$ con $L$ un Laplaciano de grafo, mientras la referencia citada (Li & Oldenburg 1998) usa primeras diferencias. | 01 §2.7 | **MEDIDO — FASE 29 (2026-09-06).** Existe el experimento que este expediente declaraba inexistente: `scripts/validation/f29_operador_suavidad.py`, factorial 2×2 (orden × frontera) sobre el dique inclinado con anti-inverse-crime ×3 y semillas pareadas. **Ningún criterio favorece al primer orden en los dos mundos**, y la nitidez del contacto —la métrica que la hipótesis señalaba— **cambia de signo**: en el mundo plano el operador de cuarto orden es MÁS nítido (3/3 semillas) y con topografía lo es el de primer orden (3/3). La discrepancia cita-vs-código **se mantiene y hay que corregir la cita**; el cambio de operador **no está justificado por el resultado**. Límites declarados: 3 semillas, un solo α (el de producción), una geometría. |
| **4** | **Una condición de contorno de Dirichlet que nadie eligió.** Restringir el Laplaciano al dominio activo por indexado deja filas que no suman cero, imponiendo contraste nulo en el aire y en el borde podado. | 01 §2.7c, 02 §2.4 | **MEDIDO — FASE 29 (2026-09-06).** El sesgo es **real y del signo predicho**: la condición de Dirichlet baja la amplitud media de la primera banda activa un **2,7 %** frente a la de Neumann, consistente en **3/3** semillas. **Pero no empeora la recuperación**: PR-AUC indistinguible (+0,2 %) y `pearson_r` **mejor** con Dirichlet en 3/3. Control exigido y cumplido: sin celdas inactivas los dos brazos salen idénticos, Δ = 0,000e+00 exacto. **Y su alcance era mayor de lo escrito**: el padding incondicional de producción (H-38) crea celdas de aire por sí solo — **2.000 de 7.200 (27,8 %)** en una corrida real —, así que la frontera está activa en **todas** las corridas, no sólo en las que declaran topografía. |
...
| **12** | **`bulk_rock_mass_kg` está en toneladas** — factor 1.000 entre el nombre de la columna y su valor (volumen m³ × densidad t/m³). Se escribe en cada corrida; sin consumidor identificado. | 06 §5.1 | Verificado. **CERRADO (Fase 22, 2026-08-27)**, y el hallazgo se quedó corto: la llamada de producción tampoco pasaba `block_size`, así que el volumen quedaba fijo en 1.000 m³ (celda de 10 m) en las **1.034 de 2.089** corridas en disco que usan otro dx, y un tope silencioso de 1e6 m³ recortaba las celdas grandes. El factor real no era 1.000 sino 1.000·dx³/1.000. Se **renombró** a `bulk_rock_mass_tonnes` —0 consumidores medidos en todo el monorepo— siguiendo el precedente del propio repositorio (`d424c7c`: `modeled_rock_mass_kg` → `modeled_rock_mass_tonnes`). |
| **13** | **`density_contrast` de la exportación se calcula contra un literal `2.6`** en vez de contra `base_density`, que es configurable. Columna homónima pero distinta de la que consume el frontend (`density_contrast_t_m3`, correcta). | 06 §5.1 | Verificado. **CERRADO (Fase 22, 2026-08-27)**, y eran **TRES** escritores, no uno: además de `gravimetry.py:4213`, el `Density_Contrast_gcm3` del `.vtr` (`export_service.VTK_BASE_DENSITY`) y el `Density_Contrast` del **ASEG-GDF2**, cuyo `.dfn` declaraba por escrito «Density minus 2.6 g/cm3 base» — y ASEG-GDF2 es formato de entrega regulatoria en Australia/NZ. Los dos últimos viajan al cliente dentro del ZIP industrial. Un cuarto sitio repetía la mentira en prosa (la nota del bloque `vtk_export` del reporte). Gate: corrida real con `base_density = 4.5` y comparación **contra el `.vtr` del disco**, no contra arrays en memoria. |
